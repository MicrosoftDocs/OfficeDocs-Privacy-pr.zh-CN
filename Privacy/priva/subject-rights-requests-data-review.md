...
查看已标识项目的列表，以确定数据主体的个人数据是否位于每个项目中。 如果项目包含数据主体的信息，并且属于数据主体的最终报告，则通过在项目列表顶部的命令栏上选择"包含"，将该项目标记为"包含"。 您还可以 **选择项目列表** 右侧内容审阅区域中的蓝色"包含"按钮。 选择" **包含"** 时，将显示一个包含添加注释的选项的飞出窗格。 完成后，选择提交以将项目的审阅状态保存为 **"包含"**。

如果不需要在请求中包括项目，请选择命令栏上的"排除"或内容审阅区域中的"排除"按钮。 排除项目意味着它与主体权限请求不相关，并且该项目不会包含在为数据主体生成的最终 [报告中](subject-rights-requests-reports.md)。

> [!NOTE]
> 如果标记项目 **"** 排除"，需要添加注释作为它与主题权限请求不相关的原因的理由。 备注仅供内部使用，不包含在最终报告中。

如果内容显示为误报，请选择"不匹配"以从最终报告中排除文件，并标记该项目为搜索中不应检测到的内容。 在"**标记为非** 匹配"飞出窗格中，选择"确认"以告诉我们该项目与搜索条件不匹配。
...
您设置和管理的五个[设置](priva-settings.md#data-review-tags)适用于所有主题权限请求。
...
- 如果对选择感到满意，请选择"保存"，以保存标记选择并关闭飞出窗格。
...
- 在项目列表上方，选择向下箭头图标，其中显示"将鼠标 **悬停在项目** 上方时导出"。
- 将Excel文件。 下载完成后打开文件。

下载的 Excel 文件显示由搜索请求收集的所有项目的属性。 查找 **"标记** "列以按标记标识和排序项目。

#### <a name="use-the-annotate-command-to-redact-text"></a>使用"注释"命令修订文本
使用 **注释** 在选定文件中创建内联标记或修订数据。 例如，如果需要包含一个还包含其他人个人信息的个人文件，可以使用命令栏) 中"绘图"按钮下的"区域修订  ("来排除与提出请求的人不相关的所有信息。 编辑完成后，选择" **包含** "将修订后的文件添加到请求中。 Annotation 创建文件的副本，以便原始文件中的内容不会更改，并且将保留在原始位置。 副本存储在 Azure blob 中。
